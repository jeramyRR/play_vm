use super::instruction::Opcode;

#[derive(Debug)]
pub struct VM {
  /// Array that simulates having hardware registers
  registers: [i32; 32],
  /// Program counter that tracks which byte is being executed
  pc: usize,
  /// The bytecode of the program being run
  program: Vec<u8>,
  /// Contains the remainder of modulo division ops
  remainder: u32,
  /// Contains the result of the last comparison operation
  equal_flag: bool,
}

impl VM {
  pub fn new() -> VM {
    VM {
      registers: [0; 32],
      pc: 0,
      program: vec![],
      remainder: 0,
      equal_flag: false,
    }
  }

  pub fn run(&mut self) {
    let mut is_done: bool = false;
    while !is_done {
      is_done = self.execute_instruction();
    }
  }

  pub fn run_once(&mut self) {
    self.execute_instruction();
  }

  pub fn execute_instruction(&mut self) -> bool {
    // If our program counter has exceeded the length of the program itself,
    // something has gone awry.
    if self.pc > self.program.len() {
      return false;
    }

    match self.decode_opcode() {
      Opcode::LOAD => {
        let register = self.next_8_bits() as usize; // We cast to usize so we can use it as an index into the array
        let number = self.next_16_bits() as u32;
        self.registers[register] = number as i32; // Our registers are i32s, so we need to cast it. We'll cover that later.
      }
      Opcode::ADD => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.registers[self.next_8_bits() as usize] = register1 + register2;
      }
      Opcode::SUB => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.registers[self.next_8_bits() as usize] = register1 - register2;
      }
      Opcode::DIV => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.registers[self.next_8_bits() as usize] = register1 / register2;
        self.remainder = (register1 % register2) as u32;
      }
      Opcode::MUL => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.registers[self.next_8_bits() as usize] = register1 * register2;
      }
      Opcode::JMP => {
        self.pc = self.registers[self.next_8_bits() as usize] as usize;
      }
      Opcode::JMPF => {
        self.pc += self.registers[self.next_8_bits() as usize] as usize;
      }
      Opcode::JMPB => {
        self.pc -= self.registers[self.next_8_bits() as usize] as usize;
      }
      Opcode::EQ => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.equal_flag = register1 == register2;
        // eat the remaining 8 bits
        self.next_8_bits();
      }
      Opcode::NEQ => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.equal_flag = register1 != register2;
        // eat the remaining 8 bits
        self.next_8_bits();
      }
      Opcode::GT => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.equal_flag = register1 > register2;
        // eat the remaining 8 bits
        self.next_8_bits();
      }
      Opcode::LT => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.equal_flag = register1 < register2;
        // eat the remaining 8 bits
        self.next_8_bits();
      }
      Opcode::GTQ => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.equal_flag = register1 >= register2;
        // eat the remaining 8 bits
        self.next_8_bits();
      }
      Opcode::LTQ => {
        let register1 = self.registers[self.next_8_bits() as usize];
        let register2 = self.registers[self.next_8_bits() as usize];
        self.equal_flag = register1 <= register2;
        // eat the remaining 8 bits
        self.next_8_bits();
      }
      Opcode::JEQ => {
        // JEQ takes one register.
        // If the equal_flag is true then it jumps to that location
        let target: usize = self.registers[self.next_8_bits() as usize] as usize;
        if self.equal_flag {
          self.pc = target;
        }
      }
      Opcode::JNEQ => {
        let target: usize = self.registers[self.next_8_bits() as usize] as usize;
        if !self.equal_flag {
          self.pc = target;
        }
      }
      Opcode::HLT => {
        println!("Halt encountered");
        return false;
      }
      Opcode::IGL => {
        println!("Unrecognized opcode found!  Terminating!");
        return false;
      }
    }
    true
  }

  fn decode_opcode(&mut self) -> Opcode {
    let opcode = Opcode::from(self.program[self.pc]);
    self.pc += 1;
    return opcode;
  }

  /// next_8_bits retrieves the index for the next 8 bits
  /// indicated by the program
  fn next_8_bits(&mut self) -> u8 {
    let result: u8 = self.program[self.pc];
    self.pc += 1;
    return result;
  }

  fn next_16_bits(&mut self) -> u16 {
    let result: u16 = ((self.program[self.pc] as u16) << 8) | self.program[self.pc + 1] as u16;
    self.pc += 2;
    return result;
  }
}

#[cfg(test)]
mod tests {
  use super::*;

  fn get_test_vm() -> VM {
    let mut test_vm = VM::new();
    test_vm.registers[0] = 5;
    test_vm.registers[1] = 10;
    test_vm
  }

  #[test]
  fn test_create_vm() {
    let test_vm = get_test_vm();
    assert_eq!(test_vm.registers[0], 5);
  }

  #[test]
  fn test_opcode_hlt() {
    let mut test_vm = get_test_vm();
    test_vm.program = vec![5, 0, 0, 0];
    test_vm.run_once();

    // test_vm.pc should be 1 because the first instruction is to halt.
    assert_eq!(test_vm.pc, 1);
  }

  #[test]
  fn test_opcode_igl() {
    let mut test_vm = get_test_vm();
    test_vm.program = vec![200, 0, 0, 0];
    test_vm.run_once();

    // test_vm.pc should be 1 because the first instruction was illegal.
    assert_eq!(test_vm.pc, 1);
  }

  #[test]
  fn test_load_opcode() {
    let mut test_vm = get_test_vm();
    test_vm.program = vec![0, 0, 1, 244]; // Remember, this is how we represent 500 using two u8s in little endian format
    test_vm.run();
    assert_eq!(test_vm.registers[0], 500);
  }

  #[test]
  fn test_jmp_opcode() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 1;
    test_vm.program = vec![6, 0, 0, 0];
    test_vm.run_once();
    assert_eq!(test_vm.pc, 1);
  }

  #[test]
  fn test_jmpf_opcode() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 2;
    test_vm.program = vec![7, 0, 0, 0, 6, 0, 0, 0];
    test_vm.run_once();
    assert_eq!(test_vm.pc, 4);
  }

  #[test]
  fn test_jmpb_opcode() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 2;
    test_vm.program = vec![8, 0, 0, 0, 6, 0, 0, 0];
    test_vm.run_once();
    assert_eq!(test_vm.pc, 0);
  }

  #[test]
  fn test_eq_opcode_for_true_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 10;
    test_vm.registers[1] = 10;
    test_vm.program = vec![9, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, true);
  }

  #[test]
  fn test_eq_opcode_for_false_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 10;
    test_vm.registers[1] = 20;
    test_vm.program = vec![9, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, false);
  }

  #[test]
  fn test_neq_opcode_for_true_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 10;
    test_vm.registers[1] = 20;
    test_vm.program = vec![10, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, true);
  }

  #[test]
  fn test_neq_opcode_for_false_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 20;
    test_vm.registers[1] = 20;
    test_vm.program = vec![10, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, false);
  }

  #[test]
  fn test_gt_opcode_for_true_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 20;
    test_vm.registers[1] = 10;
    test_vm.program = vec![11, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, true);
  }

  #[test]
  fn test_gt_opcode_for_false_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 20;
    test_vm.registers[1] = 21;
    test_vm.program = vec![11, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, false);
  }

  #[test]
  fn test_lt_opcode_for_true_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 10;
    test_vm.registers[1] = 20;
    test_vm.program = vec![12, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, true);
  }

  #[test]
  fn test_lt_opcode_for_false_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 10;
    test_vm.registers[1] = 2;
    test_vm.program = vec![12, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, false);
  }

  #[test]
  fn test_gtq_opcode_for_equal_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 20;
    test_vm.registers[1] = 20;
    test_vm.program = vec![13, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, true);
  }
  #[test]
  fn test_gtq_opcode_for_true_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 21;
    test_vm.registers[1] = 20;
    test_vm.program = vec![13, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, true);
  }

  #[test]
  fn test_gtq_opcode_for_false_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 10;
    test_vm.registers[1] = 11;
    test_vm.program = vec![13, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, false);
  }

  #[test]
  fn test_ltq_opcode_for_equal_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 20;
    test_vm.registers[1] = 20;
    test_vm.program = vec![14, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, true);
  }
  #[test]
  fn test_ltq_opcode_for_true_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 19;
    test_vm.registers[1] = 20;
    test_vm.program = vec![14, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, true);
  }

  #[test]
  fn test_ltq_opcode_for_false_value() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 11;
    test_vm.registers[1] = 10;
    test_vm.program = vec![14, 0, 1, 0];
    test_vm.run_once();
    assert_eq!(test_vm.equal_flag, false);
  }

  #[test]
  fn test_jeq_opcode() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 7;
    test_vm.equal_flag = true;
    test_vm.program = vec![15, 0, 0, 0, 17, 0, 0, 0, 17, 0, 0, 0];
    test_vm.run_once();
    assert_eq!(test_vm.pc, 7);
  }

  #[test]
  fn test_jneq_opcode() {
    let mut test_vm = get_test_vm();
    test_vm.registers[0] = 7;
    test_vm.equal_flag = false;
    test_vm.program = vec![16, 0, 0, 0, 17, 0, 0, 0, 17, 0, 0, 0];
    test_vm.run_once();
    assert_eq!(test_vm.pc, 7);
  }
}
